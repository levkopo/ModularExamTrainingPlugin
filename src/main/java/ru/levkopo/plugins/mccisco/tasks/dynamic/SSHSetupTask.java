...
public class SSHSetupTask extends DynamicTask {
    public SSHSetupTask() {
    ...
    public Task generateTask(TaskResult result) {
        ...
        return new CiscoInterfaceTask(
                ...
                "Настройте ip адрес "+ipAddress+"/"+mask+" на интерфейсе GigabitEthernet2/0. Настройте доменное имя "+domainName+". " +
                        "Включите этот интерфейс и настройте удаленный доступ по ssh v2, создав пользователя "+username+" с паролем "+password+". Дайте пользователю привелегию "+privilege,
